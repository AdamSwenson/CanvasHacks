...
class TestPointsRecord( TestingBase ):
    ...
    def test_add_fudge_points( self ):
        # prep
        points_each = self.fake.random.randint( 1, 100 )
        keys = [ self.fake.text() for _ in range( 1, 10 ) ]

        # call
        for k in keys:
            self.obj.add_fudge_points( k, points_each )

        # check
        for k in keys:
            expected = f"{self.obj.FUDGE_POINTS_PREFIX}{k}"
            self.assertIn( expected, self.obj.grade_dict.keys(), "Fudge point item in the grade_dict keys" )

    def test_total_fudge_points( self ):
        # prep
        points_each = self.fake.random.randint( 1, 100 )
        keys = [ self.fake.text() for _ in range( 1, 10 ) ]
        ...
        len(self.obj.FUDGE_POINTS_PREFIX)

        # call
        result = self.obj.total_fudge_points

        # check
        expected = points_each * len( keys )
        self.assertEqual( expected, result, "Returns sum of fudge points" )
