...
class GradeByWordCountPoints( IGradingMethodPoints):

    # METHOD_NAME = 'instructor assigned wc'

    def __init__( self, max_possible_points, threshold_dicts=[ ], required_count=None,  count_stopwords=True ):
        """
        Count dict either the thresholds via wordcounts
        and (1) percentage of total. Should be a list like:
        [
            { count : int,
            pct_credit : float
            },
        ...
        or (2) number of points. Should be a list like:
        [
            { count : int,
            points : float
            },
    ...
    def _log_message( self, wordcount ):
        self.message = f"{wordcount} words in submission"
    ...
    def grade( self, content, **kwargs ):
        ...
        self._log_message( word_count )

        # Lookup correct pct total
        for td in self.threshold_dicts:
            if word_count >= td[ 'min_count' ]:
                if 'pct_credit' in td.keys():
                    return self.max_possible_points * td[ 'pct_credit' ]
                else:
                    return td['points']
